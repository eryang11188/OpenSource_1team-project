...
**TRPG 게임**
- 본 프로젝트는 각 팀원이 독립적인 파이썬 기반 게임 기능(플레이어, 전투, 몬스터, 상점 등)을 구현한 후 GitHub 협업 전략을 통해 텍스트 기반 TRPG 시스템으로 통합하는 것을 목표로 함.
...
1. 처음 merge를 대비해서 practice 브랜치를 만들어서 병합을 로컬에서 시도해보았음
2. 이 과정에서 실수로 feature/combat 브랜치에 pracitce를 머지하고 push를 해버려서 원격 저장소까지 영향을 미침
3. 또한 confilct 충돌이 난 이유는, combat의 파일들이 다른 파일들을 import해서 사용하는 형태로 구현했지만,
4. 이것을 2개만 따로 브랜치에서 pull request를 보내 유효성 검사를 하다보니 종속적인 6개의 나머지 파일들이 없이는 검사가 실패해버렸음.
...
**우선 실행이 보장된 merge된 6개의 main 파일들을 combat 브랜치로 가져온 다음, github의 request검사를 통과하고 완성된 8개의 파일을 main으로 merge하는 절차를 밟아 해결했음.**

**그 후, Git-Hub의 원격 저장소에서 Pull Request를 통과한 feature/combat 브랜치에서 2개의 (dungeon.py, combat.py) 파일들을 로컬로 pull하였음.**
...
- merge conflict, rebase, rollback 등의 Git 문제를 직접 겪고 해결하면서 단순 명령어 암기가 아닌 문제 상황에 맞는 Git 전략 사용법을 실전처럼 학습함.

- 기능 구현보다 협업 구조와 모듈화 전략을 먼저 고려하면서, 전체 시스템의 일관성과 재사용성을 우선하는 개발 습관을 기를 수 있었음.

- 팀원 간 커뮤니케이션을 통해 작업 흐름 조율, 기능 인터페이스 통일, 중복 방지 전략을 실현하며 실질적인 협업 감각을 익힘.

# 8.2 아쉬운 점 및 개선 아이디어**

- 프로젝트 초기에는 역할 분담과 브랜치 전략에 대한 명확한 가이드가 부족해 브랜치 생성 위치나 병합 타이밍에서 충돌이 발생함. 초기 설계 단계에서 작업 흐름에 대한 구체적인 합의가 필요했음.

- 초기에 주제를 선정할 때 공학용 계산기로 선정하려 했으나, 주제의 독창성과 창의성이 부족하다고 판단되어 시간 소요가 많았음.

- 각 기능이 독립적이다 보니, 통합 시 데이터 공유 방식(예: player 객체, 인벤토리 상태 등) 을 고려하지 못해 중복된 정보 관리나 상태 동기화에 어려움을 겪음. 이후에는 전체 흐름을 고려한 공통 데이터 구조 설계가 선행되어야 할 것으로 보임.

- 실행 구조를 설계할 때 처음부터 main.py 진입 지점에서 기능을 명확히 호출하고 출력 흐름을 통제하는 방식으로 설계했다면, 통합 시 혼란을 줄일 수 있었을 것임.
